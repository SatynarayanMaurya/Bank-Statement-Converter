...
import { exportToExcelKotak } from './Export_To_Excel_Kotak';
import { exportToCSVKotak } from './Export_TO_Csv_Kotak';


function Kotak_Bank_Pdf_Data() {
...
const getNarrationParts = (narration, refNo) => {
  ...
  const handleRemarkChange = (e, refNo) => {
    ...
    const exportToExcel = ()=>{
        exportToExcelKotak(formattedData)
    }
    const exportToCSV = ()=>{
        exportToCSVKotak(pdfData)
    }
              ...
              return (
                <div key={refNo+index} className={`flex px-4 gap-6 lg:gap-0 justify-between items-center ${index%2===0 ? "bg-[#fafafa]":"bg-white"} border-[#c0c0c0] py-3`}>
...

                  {
                    isEdit ? (
                        <input type="text"
                         onChange={(e)=>handleRemarkChange(e,refNo)}
                         className='border rounded-full border-[#b5b5b5] lg:w-[12vw] w-[12rem] flex-shrink-0   py-1 px-2  outline-none cursor-pointer'
                         value={data.remark}
                         />
                    ):
...
export default Kotak_Bank_Pdf_Data
