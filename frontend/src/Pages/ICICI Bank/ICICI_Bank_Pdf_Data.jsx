...
import { exportToExcelIcici } from './Export_To_Excel_Icici';
import { exportToCSVIcici } from './Export_To_Csv_Icici';


function ICICI_Bank_Pdf_Data() {
...
const getNarrationParts = (narration, refNo) => {
  ...
  const handleRemarkChange = (e, refNo) => {
    ...
    const exportToExcel = ()=>{
        exportToExcelIcici(formattedData)
    }
    const exportToCSV = ()=>{
        exportToCSVIcici(pdfData)
    }
      ...
      <div className='overflow-x-auto '>
        <div className='  w-[95vw] lg:w-full'>
          ...
          <div className='flex justify-between gap-6 lg:gap-0 font-semibold mt-6  rounded-lg py-2 text-gray-500 px-4 '>
            ...
            <p  className=' lg:w-[30vw]  w-[18rem]  flex-shrink-0    flex gap-2 items-center'><span className='text-blue-400'><MdOutlineShortText/></span>Description</p>
          ...
          <div className='flex flex-col gap-1 mt-4   rounded-lg '>
            ...
            {pdfData?.map((data,index) => {
              return (
                <div key={data.description+index} className={`flex px-4 gap-6 lg:gap-0 justify-between items-center ${index%2===0 ? "bg-[#fafafa]":"bg-white"} border-[#c0c0c0] py-3`}>
...
export default ICICI_Bank_Pdf_Data
