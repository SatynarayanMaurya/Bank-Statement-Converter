...
import { exportToExcelIciciBusiness } from './Export_To_Excel_Icici_business';
import { exportToCSVIciciBusiness } from './Export_To_Csv_Icici_business';


function ICICI_Bank_Pdf_data_Business() {
  ...
  const handleRemarkChange = (e, refNo) => {
    ...
    const exportToExcel = ()=>{
        exportToExcelIciciBusiness(formattedData)
    }
    const exportToCSV = ()=>{
        exportToCSVIciciBusiness(pdfData)
    }
      ...
      <div className='overflow-x-auto '>
        <div className='  w-[95vw] lg:w-full'>
          ...
          <div className='flex justify-between gap-6 lg:gap-0 font-semibold mt-6  rounded-lg py-2 text-gray-500 px-4 '>
            ...
            <p  className=' lg:w-[15vw]  w-[18rem]  flex-shrink-0    flex gap-2 items-center'><span className='text-blue-400'><MdOutlineShortText/></span>Particulars</p>

            <p className=' lg:w-[8vw]  w-[7rem]  flex-shrink-0   flex gap-2 items-center'><span className='text-blue-400'>₹</span>Location</p>

            <p className='lg:w-[7vw]  w-[7rem]  flex-shrink-0   flex gap-2 items-center'><span className='text-blue-400'>₹</span>Withdrawal</p>

            <p className=' lg:w-[7vw]  w-[7rem]  flex-shrink-0   flex gap-2 items-center'><span className='text-blue-400'>₹</span>Deposit</p>

            <p className='lg:w-[7vw]   w-[12rem]  flex-shrink-0   flex gap-2 items-center'><span className='text-blue-400'><BiCategory/></span>Balance</p>
          ...
          <div className='flex flex-col gap-1 mt-4   rounded-lg '>
            ...
            {pdfData?.map((data,index) => {
              return (
                <div key={data.particulars} className={`flex px-4 gap-6 lg:gap-0 justify-between items-center ${index%2===0 ? "bg-[#fafafa]":"bg-white"} border-[#c0c0c0] py-3`}>
                  <p className='lg:w-[7vw]  flex-shrink-0   w-[7rem]  '>{data?.transactionDate}</p>

                  <p className=' lg:w-[15vw]  w-[18rem]  flex-shrink-0    overflow-scroll whitespace-nowrap hide-scrollbar  '>
                    {data?.particulars}
                  </p>

                  <p className=' lg:w-[8vw]  w-[7rem]  flex-shrink-0  overflow-scroll hide-scrollbar whitespace-nowrap '>{data?.location}</p>

                  <div className=' lg:w-[7vw]  w-[7rem]  flex-shrink-0   '>
                    {
                      data?.withdrawal && 
                      <div>
                  ...
                  {/* Category Column */}
                <p className='lg:w-[7vw]  w-[12rem]  flex-shrink-0  flex gap-2 items-center'>{data.balance}</p>
...

                  {
                    isEdit ? (
                        <input type="text"
                         onChange={(e)=>handleRemarkChange(e,data?.particulars)}
...
export default ICICI_Bank_Pdf_data_Business
