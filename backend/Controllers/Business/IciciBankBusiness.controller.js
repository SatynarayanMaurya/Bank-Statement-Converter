...
// exports.IciciBankBusinessController = async (req, res) => {
//   try {
...
exports.IciciBankBusinessController = async (req, res) => {
  try {
    ...
    // 📦 Group lines by transaction blocks
    const transactionStartRegex = /\d{2}-\d{2}-\d{4}\d{2}-\d{2}-\d{4}/g;
    const groupedTransactions = [];
    let currentGroup = [];
    ...
    // 🔍 Parse single transaction group
    function parseTransaction(group, prevBalance) {
      try {
        let flat = group.join(" ").replace(/\s+/g, " ").trim();
        ...
        if (!locKeywordMatch) {
          // console.warn("❌ No location keyword found:", flat);
          return { data: null, newPrevBalance: prevBalance };
        }

        const particulars = flat.slice(0, locKeywordMatch.index).trim();
        flat = flat.slice(locKeywordMatch.index).trim();
        ...
        const amount = parseFloat(cleanNumber(amountRaw));
        const balance = parseFloat(cleanNumber(balanceRaw));
        ...
        if (prevBalance != null) {
          if (balance > prevBalance) {
            ...
            // console.warn("⚠️ Balance unchanged, type unknown:", flat);
          }
        } else {
            withdrawal = amount;
          // console.warn("⚠️ No previous balance to compare for:", flat);
        }
        ...
        return { data, newPrevBalance: balance };
      } catch (err) {
        console.error("❌ Parsing error:", err.message, group);
        return { data: null, newPrevBalance: prevBalance };
      }
    ...
    // 🌀 Parse all transactions with running prevBalance
    let prevBalance = null;
    const transactions = [];

    for (const group of groupedTransactions) {
      const { data, newPrevBalance } = parseTransaction(group, prevBalance);
      if (data) {
